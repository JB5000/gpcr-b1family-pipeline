...
build_phylogeny.py
------------------
Build a MAFFT multiple-sequence alignment and FastTree phylogeny from the
per-species Excel workbooks generated by build_excels_from_cache.py.
"""
...
GROUP_ORDER = ["CALCR", "CRHR", "GCGR", "SCTR", "PTHR", "PDFR"]


def find_latest_run(outputs_root: str) -> Optional[str]:
    """Return the path to the most-recent run_* sub-folder, or None."""
...
def sanitize_label(value: str) -> str:
    """Convert *value* to a FASTA-label-safe string (alphanumeric + underscore)."""
    safe = re.sub(r"[^A-Za-z0-9_]+", "_", value).strip("_")
    return safe or "seq"


def normalize_for_match(value: str) -> str:
    """Lower-case *value* and strip non-alphanumeric characters for fuzzy matching."""
    return re.sub(r"[^a-z0-9]+", " ", str(value).lower()).strip()


def classify_from_description(description_line: str) -> tuple[Optional[str], Optional[str]]:
    """Assign (subfamily, family) from a PHMMER description line, or (None, None)."""
    # (original keyword-matching logic preserved below)
    ...
    if any(k in desc_norm for k in pdfr_keys):
        return "PDFR", "PDFR"
...
def collect_all_sequences(excel_path: str) -> list[tuple[str, str]]:
    """Return (label, sequence) tuples from all classified sheets in *excel_path*."""
    # Labels follow the pattern: species_accession_family_subfamily
...
def write_fasta(records: list[tuple[str, str]], output_path: str) -> None:
    """Write *records* as a multi-FASTA file to *output_path*."""
...
def run_cmd(cmd: list[str], cwd: Optional[str] = None) -> None:
    """Run *cmd* with subprocess, raising on non-zero exit."""
    subprocess.run(cmd, check=True, cwd=cwd)


def resolve_fasttree_cmd() -> str:
    """Return the name of the fasttree executable found in PATH."""
...
def build_mafft_cmd(mode: str, fasta_path: str) -> list[str]:
    """Build the MAFFT command list for the given alignment *mode*."""
    if mode == "quick":
        return [
            "mafft",
            "--retree",
...
def main() -> None:
    """Entry point: collect sequences, align with MAFFT, build FastTree phylogeny."""
    args = parse_args()
